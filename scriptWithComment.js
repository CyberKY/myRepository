...
recognition.onresult = function(event) {
    ...
    bg.style.backgroundColor = color; //styleプロパティを使用して、背景色を結果で得られた色へ更新
    console.log('Confidence: ' + event.results[0][0].confidence); //コンソールに音声認識システムの認識の正しさの信頼度を表す評価を数値で表示します。
    console.log(event.results);  
  }
  
  recognition.onspeechend = function() { //音声認識サービスにより認識された音声の停止が検出された時に発火します。
    recognition.stop(); //音声認識サービスによる入力音声のリスニングを停止し、その時点までに補足した音声を使用してSpeechRecognitionResultを返します。
  }
  
  recognition.onnomatch = function(event) { /*音声認識サービスが意味を認識できない最終結果を返した時に発火します。
  これは、ある程度の認識はされているが、信頼できるしきい値 (confidence) に達していないことを意味します。*/
